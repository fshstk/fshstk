...
public:
  ...
  {
    warningSign.applyTransform(warningSign.getTransformToScaleToFit(
      getLocalBounds().toFloat(), true, juce::Justification::centred));
...
public:
  ...
  void paint(juce::Graphics& g) override
  {
    BinauralPath.applyTransform(
      BinauralPath.getTransformToScaleToFit(0, 0, 30, 30, true, juce::Justification::centred));
...
public:
    ...
    if (availableChannels != maxPossibleNumberOfChannels) {
      ...
      if (selectable) {
        if (maxPossibleNumberOfChannels > 0)
          cbChannels->changeItemText(1, "Auto (" + juce::String(maxPossibleNumberOfChannels) + ")");
        else
  ...
  void paint(juce::Graphics& g) override
  {
    WaveformPath.applyTransform(
      WaveformPath.getTransformToScaleToFit(0, 0, 30, 30, true, juce::Justification::centred));
    g.setColour((juce::Colours::white).withMultipliedAlpha(0.5));
    ...
    if (!selectable) {
      ...
      g.setFont(15.0f);
      g.drawFittedText(
        displayTextIfNotSelectable, 35, 0, 40, 30, juce::Justification::centredLeft, 2);
    }
  }
...
public:
  ...
  void paint(juce::Graphics& g) override
  {
    AmbiLogoPath.applyTransform(
      AmbiLogoPath.getTransformToScaleToFit(0, 0, 30, 30, true, juce::Justification::centred));
    g.setColour((juce::Colours::white).withMultipliedAlpha(0.5));
...
public:
  ...
  void paint(juce::Graphics& g) override
  {
    DirectivityPath.applyTransform(
      DirectivityPath.getTransformToScaleToFit(0, 0, 30, 30, true, juce::Justification::centred));
    g.setColour((juce::Colours::white).withMultipliedAlpha(0.5));
...
public:
  ...
  void paint(juce::Graphics& g) override
  {
    ...
    juce::Rectangle<float> textArea(
      0, 0, boldWidth + regularWidth, juce::jmax(boldHeight, regularHeight));
    ...
    g.setFont(boldFont);
    g.drawFittedText(
      boldText, textArea.removeFromLeft(boldWidth).toNearestInt(), juce::Justification::bottom, 1);
...
public:
  ...
  void paint(juce::Graphics& g) override
  {
    ...
    IEMPath.applyTransform(IEMPath.getTransformToScaleToFit(
      bounds.reduced(2, 2).toFloat(), true, juce::Justification::bottomLeft));
