...
public:
  ...
  juce::Slider::SliderLayout getSliderLayout(juce::Slider& slider) override
  {
    ...
    // 1. compute the actually visible textBox size from the slider textBox size and some additional
    // constraints
    ...
    const int textBoxWidth =
      juce::jmax(0, juce::jmin(slider.getTextBoxWidth(), localBounds.getWidth() - minXSpace));
    const int textBoxHeight =
      juce::jmax(0, juce::jmin(slider.getTextBoxHeight(), localBounds.getHeight() - minYSpace));
                             ...
                             int columnFlags) override
  {
    ...
    if ((columnFlags & (juce::TableHeaderComponent::sortedForwards |
      ...
      sortArrow.addTriangle(0.0f,
                            0.0f,
                            0.5f,
                            (columnFlags & juce::TableHeaderComponent::sortedForwards) != 0 ? -0.8f
                                                                                            : 0.8f,
                            1.0f,
                            0.0f);
                                  ...
                                  juce::Slider& slider) override
  {
    ...
    juce::Colour statusColour =
      slider.findColour(juce::Slider::rotarySliderOutlineColourId).withMultipliedAlpha(0.3f);
    ...
    if (slider.isHorizontal()) {
      ...
      slbg.addRoundedRectangle(r, sliderRadius / 2.0, sliderRadius / 2.0);
      clbar.addRoundedRectangle(
        juce::Rectangle<float>(juce::Point<float>(ix + 1.0f, y + height * (1.0f - zeroPos)),
  ...
  void drawRotarySlider(juce::Graphics& g,
                        ...
                        juce::Slider& slider) override
  {
    drawRotarySliderDual(
      g, x, y, width, height, sliderPos, rotaryStartAngle, rotaryEndAngle, slider, false);
  }
                            ...
                            bool isDual)
  {
    ...
    a.addCentredArc(
      centreX, centreY, radius - extraMargin, radius - extraMargin, 0.0f, zeroAngle, angle, true);
    if (isDual)
                        ...
                        bool isButtonDown) override
  {
    if (button.getButtonText() == "ON/OFF") {
      juce::Colour baseColour(
        juce::Colours::black.withMultipliedSaturation(button.hasKeyboardFocus(true) ? 1.3f : 0.9f)
          .withMultipliedAlpha(button.isEnabled() ? 1.0f : 0.5f));
