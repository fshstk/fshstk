...
auto PluginState::getSynthParams() const -> fsh::synth::Synth::Params
{
  ...
  return {
    .voice = { .masterLevel = juce::Decibels::decibelsToGain(getParameter<float>(id(level))),
               ...
               .ampEnv = { .attack = map(ampenv_attack, 2.0f, 100.0f),
                           .decay = map(ampenv_decay, 2.0f, 100.0f),
                           .sustain = getParameter<bool>(id(ampenv_hold)) ? 0.0f : 1.0f,
                           .release = map(ampenv_decay, 2.0f, 100.0f) },
               .filtEnv = { .attack = map(filtenv_attack, 1.0f, 20.0f),
                            .decay = map(filtenv_decay, 0.0f, 2.0f),
                            .sustain = 0.0f,
                            .release = map(filtenv_decay, 0.0f, 2.0f) },
